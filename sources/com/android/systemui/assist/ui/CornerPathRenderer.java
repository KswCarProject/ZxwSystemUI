...
public abstract class CornerPathRenderer {
    ...
    public final Path approximateInnerPath(Path path, float f) {
        return toPath(shiftBy(getApproximatePoints(path), f));
    }

    public final ArrayList<PointF> getApproximatePoints(Path path) {
    ...
    public final ArrayList<PointF> shiftBy(ArrayList<PointF> arrayList, float f) {
    ...
    public final Path toPath(List<PointF> list) {
        Path path = new Path();
        if (list.size() > 0) {
    ...
    public final PointF normalAt(List<PointF> list, int i) {
        PointF pointF;
        PointF pointF2;
        if (i == 0) {
    ...
    public final PointF rotate90Ccw(PointF pointF) {
        return new PointF(-pointF.y, pointF.x);
    }

    public final float magnitude(PointF pointF) {
        float f = pointF.x;
        float f2 = pointF.y;
        return (float) Math.sqrt((double) ((f * f) + (f2 * f2)));
    }

    public final PointF normalize(PointF pointF) {
